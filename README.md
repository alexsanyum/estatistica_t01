...
O TLC estabelece que para o caso da média amostral, ela se ajusta a $`\mathbb{N} (\mu, \sigma^{2}/ n)`$. É dizer, à medida que $`n`$ aumenta, a média se mantém e a variância diminui. Isso pode ser comprovado em todos os casos. Nas Figuras 2-5 (parte superior), podemos observar que à medida que $`n`$ aumenta, a variância vai diminuindo. De fato, a variância diminui tanto que para os histogramas de $`n=50`$ elas não se destacam, já que elas são muito próximas ao valor da média e isso não permite distingui-las visualmente. Esta observação nos permite confirmar a teoria do TLC. À medida que $`n`$ cresce, temos maior probabilidade de tomar valores tão próximos da média real.
O TLC nos permite ver que não importa a distribuição original de dados, se temos um número grande de realizações/amostras/pontos, podemos aproximar-nos da média e da variância original dos dados ainda sem conhecer como eles são realmente. No enunciado **(f)** vimos que o conceito de $`n`$ grande varia de acordo com a complexidade dos dados. Para os casos de $`\overline{Q}_{n}`$, $`\overline{X}_{n}`$, e $`\overline{T}_{n}`$, com $`n=5`$ as médias amostrais se aproximavam da normal teórica, enquanto para $`\overline{Y}_{n}`$, esta se aproxima melhor com $`n=50`$. O TLC sempre se cumprirá à medida que $`n`$ aumenta; quão "grande" dependerá do caso. 
